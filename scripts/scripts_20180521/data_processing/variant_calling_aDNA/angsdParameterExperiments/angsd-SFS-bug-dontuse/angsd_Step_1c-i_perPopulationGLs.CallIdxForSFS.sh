...
-uniqueOnly 1 \
-doMaf 2 \
-out $GLdir/$todaysdate/perPopulation/${mfurBamList%.bamList.*}.mappedToMfur.${snpCutoff}.snpsOnly \
-SNP_pval 1e-6 \
-remove_bads 1 \
-C 50 
...
-out $SFSdir/$todaysdate/${mfurBamList%.bamList.*}.mappedToMfur.${snpCutoff}.snpsOnly.TransversionsOnly \
-nInd $nInd

# note here nInd is 3!! not 15!!!

realSFS $SFSdir/$todaysdate/${mfurBamList%.bamList.*}.mappedToMfur.${snpCutoff}.snpsOnly.TransversionsOnly.saf.idx > $SFSdir/$todaysdate/${mfurBamList%.bamList.*}.mappedToMfur.${snpCutoff}.snpsOnly.TransversionsOnly.saf.SFS.txt
...
-out $SFSdir/$todaysdate/${elutBamList%.bamList.*}.mappedToElut.${snpCutoff}.snpsOnly.TransversionsOnly\
-nInd $nInd 

# note here nInd is 3!! not 15!!!

realSFS $SFSdir/$todaysdate/${elutBamList%.bamList.*}.mappedToElut.${snpCutoff}.snpsOnly.TransversionsOnly.saf.idx > $SFSdir/$todaysdate/${elutBamList%.bamList.*}.mappedToElut.${snpCutoff}.snpsOnly.TransversionsOnly.saf.SFS.txt
# evnetually want to use -r to make SFSes for different regions from this idx file

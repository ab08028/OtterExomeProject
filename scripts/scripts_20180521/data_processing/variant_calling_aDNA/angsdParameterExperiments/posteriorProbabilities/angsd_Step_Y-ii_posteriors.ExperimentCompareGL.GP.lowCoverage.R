...
rundate="20190522"
######## dataset 1: high coverage modern + aDNA just for scaffold GL896898.1############
#state="highcov"

######## dataset 2: low coverage (downsampled) modern + aDNA just for scaffold GL896898.1 ############
state="lowcov"
#lowCovSampList=read.table(paste(sampleListDir,"SampleIDsInOrder.LowCoverageOnly.BeCarefulOfOrder.txt",sep=""))
...
GLvsGP_plot2
ggsave(paste(data.dir,"meanGL.vs.meanGP.OneScaffold.2.png",sep=""),GLvsGP_plot2,width=10,height=7)

################ add in MAFS #########
head(MAFs)
# make a marker column:
MAFs$marker=as.factor(paste(MAFs$chromo,MAFs$position,sep="_"))
meanGL_GP_MAFs = inner_join(meanGL_GP,MAFs,by="marker")

# plot difference between GP and GL 
# positive difference is gain in confidence
# negative difference is loss in confidence
# vs number of 
GLGPMAFPlot <-ggplot(meanGL_GP_MAFs,aes(x=nInd,y=(meanGP-meanGL),color=label))+
  geom_point()+
  facet_wrap(~GTLabel)
GLGPMAFPlot
ggsave(paste(data.dir,state,".MeanGP-GL.vsNumInd.OneScaffold.png",sep=""),GLGPMAFPlot,width=10,height=7)

GLGPMAFPlot2 <-ggplot(meanGL_GP_MAFs,aes(x=knownEM,y=(meanGP-meanGL),color=label))+
  geom_point()+
  facet_wrap(~GTLabel)
GLGPMAFPlot2
ggsave(paste(data.dir,state,".MeanGP-GL.vsMAF.OneScaffold.png",sep=""),GLGPMAFPlot2,width=10,height=7)

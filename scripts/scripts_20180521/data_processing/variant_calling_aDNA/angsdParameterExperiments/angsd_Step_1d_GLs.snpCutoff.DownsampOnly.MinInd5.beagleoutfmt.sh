...
minInd=5 # must be info for at least this many individuals. from Daly: 1/2 the sample size, rounded up. so if I have 9 downsampled samples, 1/2 is 5
...
-uniqueOnly 1 \
-doMaf 2 \
-out $GLdir/$todaysdate/angsdOut.mappedToMfur.${snpCutoff}.snpsOnly.downSampOnly.minInd.${minInd} \
-SNP_pval 1e-6 \
-remove_bads 1 \
-C 50 \
-minInd $minInd
...
-uniqueOnly 1 \
-doMaf 2 \
-out $GLdir/$todaysdate/angsdOut.mappedToElut.${snpCutoff}.snpsOnly.downSampOnly.minInd.${minInd} \
-SNP_pval 1e-6 \
-remove_bads 1 \
-C 50 \
-minInd $minInd

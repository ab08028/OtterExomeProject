...
qsub $scriptDir/${script} $scriptDir/bamLists/$sampleID.$refPrefix.bamList.txt $label $refPrefix $mfurRef $minDepth
done
...
label=${filename%.newSampName.*} # gets info from filename 
echo $bam > $scriptDir/bamLists/$sampleID.$refPrefix.bamList.txt
qsub $scriptDir/${script} $scriptDir/bamLists/$sampleID.$refPrefix.bamList.txt $label $refPrefix $elutRef $minDepth
done

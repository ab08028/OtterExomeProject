...
######### Step 4 b: calls pseudohaploids by randomly sampling a single read at each site per individual (after applying all filters)
# based on downsampled low coverage modern + aDNA mapped to elut/mfur ######
...
todaysdate=`date +%Y%m%d`'-lowcov-pseudoHaps'
...
echo -e "THIS USES LOW COVERAGE DOWNSAMPLED MODERN + ANCIENT ONLY\nBamLists used:\n$elutBamList\n$mfurBamList \ntrimvalue = $trimValue\nPSEUDOHAPLOIDS SAMPLING ONE RANDOM READ" > $HAPdir/$todaysdate/LOWCOVERAGEONLY.txt
...
# need to define the missing genotype as "N" otherwise it thinks sites are triallelic
...
# note skipTriallelic doesn't work in angsd here, so I do it with a custom script
# you have to do doCounts for it to work, but I'm skipping -dumpCounts 2
# don't really want the counts output from this stage, it's not useful
# I want to exclude triallelic sites and filter transversions:

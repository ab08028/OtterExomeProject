...
headers=$SCRATCH/captures/samples/ancient.modern.comparison.txt # ancient and modern
outdir=$SCRATCH/captures/paleomix/summaryStats
mkdir $outdir
todaysdate=`date +%Y%m%d`
outfile=$outdir/plmx.aDNA.summary.stats.fullProcessing.ancient.modern.comparison.$todaysdate.txt
...
grep -w seq_retained_nts $wd/$header/*summary | awk '{print($1,$4,"noREF",$5)}' | sort | uniq >> $outfile

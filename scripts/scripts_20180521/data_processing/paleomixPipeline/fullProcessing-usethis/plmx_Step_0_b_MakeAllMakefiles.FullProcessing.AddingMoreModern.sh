...
modernTemplate=$makefileDir/makefile_template.modernDNA.IndelR.toCompareToAncientOnly.yaml # only difference from my main modern analysis is that indel realignment is done 
#ancientTemplate=$makefileDir/makefile_template.aDNA.fullProcessing.IndelR.ElutMfur.yaml
#ancHeaders=$wd/samples/bestAncientHeaders.txt # 3 best aDNAs  # # A13 has _L_ , A29-30 don't


#modHeaders=$wd/samples/modern.SamplesToCompareToADNA.txt # choose 3 CA and 3 AK to process; want to downsample at some point along the way (maybe at BAM stage)
### 20191212: adding more samples from each of the other populations 
modHeaders=$wd/samples/modern.SamplesToCompareToADNA.COM.AL.KUR.txt

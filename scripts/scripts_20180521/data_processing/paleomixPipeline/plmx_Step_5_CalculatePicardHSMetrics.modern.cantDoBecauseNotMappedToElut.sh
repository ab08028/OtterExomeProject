...
headers=/u/flashscratch/a/ab08028/captures/samples/modernSamples.plus.capture_02.plus.Baja.txt
...
# convert
refDir=/u/home/a/ab08028/klohmueldata/annabel_data/ferret_genome/
...
java -jar $PICARD BedToIntervalList \
       ...
       SD=$refDict
plmxDir=/u/flashscratch/a/ab08028/captures/paleomix/
...
grep -A1 "BAIT_SET" ${plmxBam%.bam}.hs_metrics.region2target.txt | grep -v "BAIT_SET" | awk -F '\t' -v header=$header '{OFS="\t";print $0, header}'  | tr -s '\t' '\t' >> $sumStatDir/picard.HSMetrics.plmx.modern.${todaysdate}.txt
done

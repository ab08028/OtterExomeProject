...
#$ -m bea
#$ -M ab08028
#$ -N fq2sam_submit_modern
######### This script run in the shell will submit a series of jobs that convert fastq to sam and adds readgroup info
...
$QSUB -e $errorLocation -o $errorLocation -M $user -N fq2sam${c} $scriptDir/generic/$scriptname $fastqs/$fileR1 $fastqs/$fileR2 $bams/${header}_FastqToSam.bam ${header}_1a ${header} Lib1 $UNIT $platform $seqCenter

...
outdir=$wd/checkFilters
mkdir -p $outdir 
zcat all_5_passingFilters_80percCall_${infile} | grep -v "#" | awk 'length($4)>1 || length($5)>1' > $outdir/indelLines.all.test.vcf
zcat snp_5_passingAllFilters_postMerge_raw_variants.${infile} | grep -v "#" | awk 'length($4)>1 || length($5)>1' > $outdir/indelLines.snps.vcf

...
stats <- read.table(paste(resultsDir,"QC_Reports/plmx_mappingStats/",date,"_allADNA/plmx.aDNA.summary.stats.allAncient.",date,".txt",sep=""),header=T,stringsAsFactors = F)
...
statsToPlot1 <- stats[stats$statistic!="hits_raw" & stats$statistic!="hits_raw_frac"  & !(stats$statistic %in% c("MD_001","MD_002","MD_003")),]
statsToPlot1$statistic <- factor(statsToPlot1$statistic)
levels(statsToPlot1$statistic) <- c("PCR Duplicate (frac)","Mapped Read Length\n(bp)", "Unique Mapped Reads\n(read count)","Unique Mapped Reads\n(frac)","Mb >= 1x","Mb >= 2x","Mb >= 3x","Sequencing Bases","Sequencing Reads")
statsToPlot1$plotOrder <- 2 # some lower number
statsToPlot1[statsToPlot1$sample %in% c("A12_Blank_Mycroarray_T19"),]$plotOrder <- 1
statsToPlot1[statsToPlot1$sample %in% c("A9_Elut_CA_AN_388_SN1","A10_Elut_CA_NIC_1_SN1"),]$plotOrder <- 3
### make this work for headers for Beth report ** YOU ARE HERE ** 
# just plot seq retained reads:
p0 <- ggplot(data=statsToPlot1[statsToPlot1$statistic=="Sequencing Reads",],aes(x=reorder(sample,plotOrder),y=value))+
...
p0
ggsave(paste(plotoutDir,"/retainedReads.",todaysdate,".pdf",sep=""),p0,device="pdf",width = 8,height=5)

######### 0b Plot the Mb of genome coverage at 1/2x per sample #########
p0b <- ggplot(data=statsToPlot1[statsToPlot1$statistic %in% c("Mb >= 1x","Mb >= 2x","Mb >= 3x"),],aes(x=reorder(sample,plotOrder),y=value,fill=statistic))+
  geom_bar(stat="identity",position = "dodge")+
  ylab("Mb of sea otter genome")+
  theme_bw()+
  coord_flip()+
  xlab("Sample")+
  theme(axis.text=element_text(size=14),axis.title=element_text(size=14))
p0b
ggsave(paste(plotoutDir,"/MbSeaOtterGenome_1xCoverage.",todaysdate,".pdf",sep=""),p0b,device="pdf",width = 8,height=5)

######### 0c Plot the Mapping Read count and frac #########

p0c <- ggplot(data=statsToPlot1[statsToPlot1$statistic %in% c( "Unique Mapped Reads\n(read count)","Unique Mapped Reads\n(frac)"),],aes(x=reorder(sample,plotOrder),y=value,fill=simpleRef))+
  geom_bar(stat="identity",position = "dodge")+
  ylab("")+
  theme_bw()+
  coord_flip()+
  theme(axis.text=element_text(size=14),axis.title=element_text(size=14),legend.text = element_text(size=14),legend.title = element_blank(),strip.text.x = element_text(size=14))+
  facet_wrap(~statistic,scales="free_x")+
  xlab("")+
  ggtitle("")
  
p0c
ggsave(paste(plotoutDir,"/MappedReads",todaysdate,".pdf",sep=""),p0c,device="pdf",width = 11,height=8)

######### 0d Plot the PCR Duplicate Rate and  #########

p0d <- ggplot(data=statsToPlot1[statsToPlot1$statistic %in% c("PCR Duplicate (frac)","Mapped Read Length\n(bp)"),],aes(x=reorder(sample,plotOrder),y=value,fill=simpleRef))+
  geom_bar(stat="identity",position = "dodge")+
  ylab("")+
  theme_bw()+
  coord_flip()+
...
p0d
ggsave(paste(plotoutDir,"/DuplicateRate_and_Length",todaysdate,".pdf",sep=""),p0d,device="pdf",width = 11,height=8)
...
statsToPlot <- stats[stats$statistic!="seq_retained_reads" & stats$statistic!="hits_raw" & stats$statistic!="hits_raw_frac"  & !(stats$statistic %in% c("MD_001","MD_002","MD_003")) & !(stats$statistic %in% c("Mb >= 1x","Mb >= 2x","Mb >= 3x","seq_retained_nts")),]
...
ggsave(paste(plotoutDir,"aDNAMappingStats",todaysdate,".pdf",sep=""),p1_grid,device="pdf",width = 32,height=16)
...
p1b <- ggplot(data=statsToPlot[statsToPlot$simpleRef=="sea otter",],aes(x=reorder(sample,value),y=value))+
  ...
  theme_bw()
p1b
ggsave(paste(plotoutDir,"aDNA.AllSamples.Stats.ElutRef.",todaysdate,".pdf",sep=""),p1b,device="pdf",width = 8,height=8)
...
p2
ggsave(paste(plotoutDir,"aDNA_ReadCount-vs-PercMapped.",todaysdate,".pdf",sep=""),p2,device="pdf",width = 8,height=8)
...
p3
ggsave(paste(plotoutDir,"aDNA_compareToAvilaArcosStudy.",todaysdate,".pdf",sep=""),p3,device="pdf",width = 8,height=8)

...
# for now need to exclude RWAB

samplesToExclude <- c("149_Elut_CA_207","148_Elut_CA_164","147_Elut_CA_389","146_Elut_CA_375") 
popmap <- popmap[grep("RWAB",popmap$Sample,invert = T) & !(popmap$Sample %in% samplesToExclude),]
splitSampleNames <- split(popmap$Sample,popmap$PrimaryPop) # this is a list of dataframes # this works well
...
ibdFunc <- function(genofile,snpset.id,sample.id.list){
  ibs.robust <- snpgdsIBDKING(genofile,autosome.only = F,remove.monosnp = T,missing.rate = 0.2, maf=0.05,snp.id=snpset.id,sample.id = sample.id.list)
  dat <- snpgdsIBDSelection(ibs.robust)
  return(dat)
}
#ibdFunc(genofile,snpset.id,CA.id) # need to get rid of 4 ca samples with low coverage
dat.ibs.allPops <- lapply(splitSampleNames,ibdFunc,genofile=genofile,snpset.id=snpset.id)
# worked!
#sink()
names(dat.ibs.allPops)
# close genofile
...
# https://stackoverflow.com/questions/42588761/how-can-i-make-multiple-ggplots-from-a-list-of-dataframes-and-have-them-on-one-p?rq=1

p1List <- lapply(dat.ibs.allPops, function(d) ggplot(data=d,aes(x=IBS0,y=kinship))+
                   theme_bw()+
                 geom_point())
myplots <- list()
for (i in seq(dat.ibs.allPops)) {
  gg <- ggplot(dat.ibs.allPops[[i]], aes(x = IBS0, y = kinship))+
    geom_point() +
    ggtitle(names(dat.ibs.allPops[i]))
  print(gg)
  myplots[[i]] <- gg
}
require(gridExtra)
...
dat2 <- merge(dat.ibs.allPops,popmap,by.x="ID1",by.y="Sample")

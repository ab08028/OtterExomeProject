...
#models=c("dadiModel1.OldShallowContraction",  "dadiModel2.RecentExtremeContraction")
#replicate=1
############################### model 1 #############################
# this is the actual demographic model
model=model1
model1Rect1=data.frame(xmin=0,xmax=1000,ymin=0,ymax=2000)
model1Rect2=data.frame(xmin=1000,xmax=30000,ymin=0,ymax=4500)
model1_allReps <- data.frame()
...
# note now this result is labeled with its rep number (in the function), so can tell apart
    model1_allReps = rbind(model1_allReps,results_scaled)
  }
  else{ print(paste("rep",replicate," does not exist!"))}
}
p1 <- ggplot(model1_allReps)+
  geom_rect(data=model1Rect1,aes(xmin=xmin,xmax=xmax,ymin=ymin,ymax=ymax),fill="purple",alpha=0.4)+
  geom_rect(data=model1Rect2,aes(xmin=xmin,xmax=xmax,ymin=ymin,ymax=ymax),fill="purple",alpha=0.4)+
...
############################### model 2 #############################
model=model2
model2Rect1=data.frame(xmin=0,xmax=35,ymin=0,ymax=195)
model2Rect2=data.frame(xmin=35,xmax=30000,ymin=0,ymax=3500)
model2_allReps <- data.frame()
for(replicate in c(seq(1,10))){
  # check if file is empty; skip if it is
  if(file.exists(paste(data.dir,model,"/rep_",replicate,"/msmc.RunOn.",model,".sims.out.final.txt",sep=""))){
    results <- read.table(paste(data.dir,model,"/rep_",replicate,"/msmc.RunOn.",model,".sims.out.final.txt",sep=""),header=T)
    results_scaled <- scaleMSMC(results,mu,gen=7,model,paste("rep_",replicate,sep=""),category="main")
    # note now this result is labeled with its rep number (in the function), so can tell apart
    model2_allReps = rbind(model2_allReps,results_scaled)
  }
  else{ print(paste("rep",replicate," does not exist!"))}
}
p2 <- ggplot(model2_allReps)+
  geom_rect(data=model2Rect1,aes(xmin=xmin,xmax=xmax,ymin=ymin,ymax=ymax),fill="purple",alpha=0.4)+
  geom_rect(data=model2Rect2,aes(xmin=xmin,xmax=xmax,ymin=ymin,ymax=ymax),fill="purple",alpha=0.4)+
  geom_step(stat="identity",size=0.5,aes(x=Left_generations,y=Ne,group=label),color="black")+
  theme_bw()+
  theme(legend.title = element_blank())+
  ggtitle(model)+

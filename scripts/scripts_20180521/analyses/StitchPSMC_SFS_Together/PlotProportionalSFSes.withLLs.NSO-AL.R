...
require(ggplot2)
########## empirical sfs ########
data <- read.table("/Users/annabelbeichman/Documents/UCLA/Otters/OtterExomeProject/results/datafiles/SFS/20181119/easySFS_projection/neutral/projection-20181221-hetFilter-0.75/dadi-plusMonomorphic/AL-21.plusMonomorphic.sfs",skip=1,stringsAsFactors = F)
numHap=dim(data)[2]-1
numDip=numHap/2
...
data.dir="/Users/annabelbeichman/Documents/UCLA/Otters/OtterExomeProject/results/analysisResults/StitchPSMC_SFS_Together/NSO-AL-Comparison/"
for(model in models){
...
# this has all pops
# want CA and T ~ 35
twoEpochGrid[twoEpochGrid$population=="AL" & twoEpochGrid$T_scaledByNanc_from_Theta_gen <= 36 & twoEpochGrid$T_scaledByNanc_from_Theta_gen >= 35,]
# gives you two to choose from. Want row 1588 (nu = 433, T = 35.63)
AL_BestDadiModel <- twoEpochGrid[1588,] # choose row 252 
AL_BestDadiModel_expSFS <- as.numeric(unlist(strsplit(as.character(AL_BestDadiModel$expectedSFS_fold_Theta1),", "))[2:(numDip+1)]) # this is folded and is relative to theta =1 , pull out the entries 1-6 (excluding first term which is monomorphic and masked)
AL_BestDadiModel_expSFS_freq <- data.frame(AL_BestDadiModel_expSFS/sum(AL_BestDadiModel_expSFS))
colnames(AL_BestDadiModel_expSFS_freq) <- "proportion"
AL_BestDadiModel_expSFS_freq$freq <- as.numeric(seq(1,numDip))
AL_BestDadiModel_expSFS_freq$label <- "best dadi model"
...
######### get LL:
combo2 <- rbind(AL_BestDadiModel_expSFS_freq,data_melt_noMono[,c("label","freq","proportion")])
#### plot with empirical:

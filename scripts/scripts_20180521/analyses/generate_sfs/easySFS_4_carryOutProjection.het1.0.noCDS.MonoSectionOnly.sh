#! /bin/bash $ -cwd $ -l h_rt=10:00:00,h_data=20G,highp $ -N easySFSProjection $ -o 
#/u/flashscratch/a/ab08028/captures/reports/SFS $ -e /u/flashscratch/a/ab08028/captures/reports/SFS $ -m 
#abe $ -M ab08028
...
allSamplesHetFilter=0.75 # het filtering done across all samples; keep the same for now, but making per pop less stringent
perPopHetFilter=1 # the vcf file has already had some degree of maxHetFiltering. now per-population, easy sfs will do per-population filtering at this level  
# after troubleshooting and looking at HWE exact test, 0.75 was chosen
# but in jan 2020 I am wondering if that is too stringent, so running this test

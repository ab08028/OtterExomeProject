...
projectionValues= [int(x) for x in args.projections.split(",")]
...
# popMap="/Users/annabelbeichman/Documents/UCLA/Otters/OtterExomeProject/information/samples/samplesPop.Headers.forEasySFS.2.goingtoModifyFor20181119.txt" ### not final version of this!!!!! <--- but use for now for testing. 
...
outputFile=open(str(outdir)+"/"+prefix+".countsOfMonomorphicPassingProjectionThresholds."+todaysdate+".txt","w")
outputFile.write("population\tHomREFcount\tProjectionValue\n")
[outputFile.write('{0}\t{1}\t'+str(projDict[{0}])+'\n').format(key,value) for key, value in counts.items()]
outputFile.close()

...
parser.add_argument("--TB_Low",required=True,help="input lower bound on TB (duration of bneck period; rec duration is fixed at 10 gen) in dadi units ; distance between T_Low and T_High will be searched evenly along a log scale")
parser.add_argument("--TB_High",required=True,help="input upper bound on TB (duration of bneck period; rec duration is fixed at 10 gen) in dadi units; distance between low and high will be searched evenly along a log scale")
...
TB_High_rescaled=float(args.TB_High)
TB_Low_rescaled=float(args.TB_Low)
maxiter=100
...
TF_fixed=0.001 # fixed recovery time at ~10 gens 
...
# allow bneck duration to be inferred:
TBs= np.logspace(math.log10(TB_Low_rescaled),math.log10(TB_High_rescaled),numGridPoints) # goes from approx 1 gen to ~70 gen
# set up your set of parameters
param_grid = {'nuB': nuBs, 'nuF' : nuFs, 'TB' : TBs,}
...
for params in grid:
    #print(params)
    output=provideParams_3EpochModel(fs=fs,nuB=params['nuB'],nuF=params['nuF'],TF=TF_fixed,TB=params['TB'])

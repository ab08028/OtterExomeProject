...
p1
ggsave(paste(plotoutdir,"/PCA.inclCA.LDPruned.postRelativeRemoval.AdmixedRemoval.",todaysdate,".pdf",sep=""),p1,device="pdf",width = 8,height=5)
...
p2 <- ggplot(tab2,aes(x=EV1,y=EV2,color=pop1,shape=sequencer,label=sample.id))+
  ...
  ggtitle(paste("PCA based on ",as.character(length(pca$snp.id))," LD Pruned SNPs",sep=""))
p2
ggsave(paste(plotoutdir,"/PCA.excludeCA.LDPruned.postRelativeRemoval.AdmixedRemoval.",todaysdate,".pdf",sep=""),p2,device="pdf",width = 8,height=5)
...
p3
ggsave(paste(plotoutdir,"/PCA.excludeCA.LDPruned.postRelativeRemoval.AdmixedRemoval.withNames.",todaysdate,".pdf",sep=""),p3,device="pdf",width = 16,height=10)

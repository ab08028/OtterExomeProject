...
if [ $outgroup = "NoOutgroup" ]; then
	dowork() { 
		...
		b=1`date +%N`
		let "c=$a+$b"
		treemix -bootstrap -i $2 -k $3 -se -m $4 -seed $c -o $outdir"/bootstrap/"$5"_treemix_bootrep_"$1
	}
	export -f dowork
...
echo "outgroup is: " $outgroup
	dowork() { 
		...
		b=1`date +%N`
		let "c=$a+$b"
		treemix -bootstrap -i $2 -k $3 -se -m $4 -root $6 -seed $c -o $outdir/"bootstrap/"$5"_treemix_bootrep_"$1
	}
	export -f dowork
	for i in `seq 1 $nboot`
	do
...
if [ $outgroup = "NoOutgroup" ]; then
	treemix -i $infile -m $numk -k $blockk -se -tf $outdir/$outname"_outtree.newick" -o $outdir/${outname}.tfConsensus > $outdir/$outname"_logfile_treemix_boot.log"
else
	treemix -i $infile -m $numk -k $blockk -root $outgroup -se -tf $outdir/$outname"_outtree.newick" -o $outdir/${outname}.tfConsensus > $outdir/$outname"_logfile_treemix_boot.log"
fi
echo "**** RUNNING TREEMIX: DONE ****"

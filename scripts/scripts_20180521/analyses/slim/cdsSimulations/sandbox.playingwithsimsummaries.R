################### sandbox to play with sim output ############
require(ggplot2)
require(dplyr)
...
popModDates="AK/1D.2Epoch.1.5Mb.cds.LongerContract/20190607"
...
avgHomPerIndPersCat <- allInputs %>%
  group_by(state,h,population,sCat,replicate,popModDate) %>%
...
p1 <- ggplot(avgHomPerIndPersCat,aes(y=n,x=state,fill=state,group=interaction(popModDate,state)))+
  geom_violin(position=position_dodge(0.5))+
  geom_point(position=position_dodge(0.5),size=0.3)+
  theme_bw()+
  facet_grid(sCat~h,scales="free")+
  theme(legend.position = "NA")+
  ggtitle("Average number of homozygous derived genotypes per individual, per category\nneutral [0]; weak (0 to -0.001]; moderate (-0.001 to -0.01]; strong (-0.01 to -1]")

...
popModsDate=c("AK/1D.2Epoch.1.5Mb.cds/20190424/", "AK/1D.2Epoch.1.5Mb.cds.LongerContract/20190607/") 
popmodels=list()
...
countMeanPlot1 <- ggplot(summarizedSFSes,aes(x=frequency,y=count_MEAN,fill=state,group=internalID))+
  geom_bar(stat="identity",position="dodge",alpha=0.75)+
  geom_errorbar(aes(ymin=count_MEAN-count_SE, ymax=count_MEAN+count_SE), width=.2,
...
########### Plot Average with +-1 SD ###############
################# plotting longer contraction vs shorter contraction ############
countMeanPlot2 <- ggplot(summarizedSFSes,aes(x=frequency,y=count_MEAN,fill=model,group=internalID))+
  geom_bar(stat="identity",position="dodge",alpha=0.75)+
  geom_errorbar(aes(ymin=count_MEAN-count_SE, ymax=count_MEAN+count_SE), width=.2,
                position=position_dodge(0.9)) +
  scale_fill_manual(values=c("blue","darkred"))+
  facet_grid(population~mutLabel~dominance_h~state)+
  theme_bw()+
  scale_x_continuous(breaks=c(seq(1,max(as.numeric(summarizedSFSes$frequency)))))+
  ggtitle("Simulated Alaska SFS averaged over simluation replicates\nError Bars denote one standard error\nComparing two versions of the contraction\nwith shorter or longer contraction times\n(Basic model is 30 inds. for 4 gen; LongerContract is 250 inds for 35 gen.)")
countMeanPlot2
ggsave(paste(plot.dir,"LongerVsShorterContractionYieldSameSFS.AK.",todaysdate,".pdf",sep=""),countMeanPlot2,device="pdf",width=7,height=5)
...
EMPpopsIWant <- "AK"
SIMpopsIWant <- "AK"
SIMmodelsIWant <- "1D.2Epoch.1.5Mb.cds.LongerContract"
...
p1a <- ggplot(combo.cds.h_0[combo.cds.h_0$mutLabel!="neutral",],aes(x=as.numeric(frequency),y=as.numeric(proportion),fill=state,group=interaction(popModel,state)))+
  ...
  facet_grid(as.factor(popLabel2)~mutLabel,scales="free")+
  ...
  ggtitle(paste("Dominance Coefficient (h): 0\nSimulation info: ", unique(combo.cds.h_0[combo.cds.h_0$category!="empirical",]$popModel),"\nError Bars denote one standard error",sep=""))+
  theme(legend.position=c(0.3,0.8),legend.background = element_rect("transparent"),legend.title=element_blank(),legend.key.size = unit(1,"cm"),legend.text = element_text(size=14))
p1a
...
p1b <- ggplot(combo.cds.h_0.5[combo.cds.h_0.5$mutLabel!="neutral",],aes(x=as.numeric(frequency),y=as.numeric(proportion),fill=state,group=interaction(popModel,state)))+
  ...
  ylab("Proportion")+
  scale_x_continuous(breaks=c(seq(1,max(as.numeric(combo.cds.h_0$frequency)))))+
  ggtitle(paste("Dominance Coefficient (h): 0.5\nSimulation info: ", unique(combo.cds.h_0.5[combo.cds.h_0.5$category!="empirical",]$popModel),"\nError Bars denote one standard error",sep=""))+
  theme(legend.position=c(0.3,0.8),legend.background = element_rect("transparent"),legend.title=element_blank(),legend.key.size = unit(1,"cm"),legend.text = element_text(size=14))
p1b
# you'll get this warning:
#Warning message:
...
p1c <- ggplot(additiveRecessiveMissense,aes(x=as.numeric(frequency),y=as.numeric(proportion),fill=dominance_h))+
  ...
  scale_x_continuous(breaks=c(seq(1,max(as.numeric(combo.cds.h_0$frequency)))))+
  ggtitle(paste("Dominance Coefficient (h): 0.5\nError Bars denote one standard error",sep=""))+
  theme(legend.position=c(0.3,0.85),legend.background = element_rect("transparent"),legend.title=element_blank(),legend.key.size = unit(1,"cm"),legend.text = element_text(size=14))
p1c
# you'll get this warning:
#Warning message:
#  Removed 14 rows containing missing values (geom_errorbar). 
# because empirical doesn't have error bars' == that's the expected behavior.
ggsave(paste(plot.dir,"ComparingFitOfAdditive.Recessive.MissenseToEmpirica.",todaysdate,".pdf",sep=""),p1c,device="pdf",width=11,height=5)

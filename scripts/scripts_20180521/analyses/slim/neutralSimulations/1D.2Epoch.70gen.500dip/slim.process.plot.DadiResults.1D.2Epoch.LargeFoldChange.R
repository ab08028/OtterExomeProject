...
nu_true=500 # nu you simulated
T_true=70 # gen you simulated
...
dadimodel="1D.2Epoch.LargeFoldChange" # model you inferred in dadi
...
p1 <- ggplot(allresults,aes(x=reorder(rep, sort(as.numeric(rep))),y=Nanc_FromTheta_scaled_dip,color=deltaLL))+
  geom_point()+
  ...
  xlab("Replicate")
p1
p2 <- ggplot(allresults,aes(x=reorder(rep, sort(as.numeric(rep))),y=T_scaled_gen,color=deltaLL))+
  geom_point()+
  ...
  xlab("Replicate")
p2 
p3 <- ggplot(allresults,aes(x=reorder(rep, sort(as.numeric(rep))),y=nu_scaled_dip,color=deltaLL))+
  geom_point()+
...
# simulated values 
allresults$trueT <- 70 / (2*allresults$Nanc_FromTheta_scaled_dip)
allresults$trueNu <- 500 / allresults$Nanc_FromTheta_scaled_dip

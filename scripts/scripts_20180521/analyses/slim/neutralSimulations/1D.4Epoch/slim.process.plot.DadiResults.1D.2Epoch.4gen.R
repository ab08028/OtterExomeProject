...
slimmodel="1D.2Epoch.4gen" # model you simulated under
slimdate=20190128 # date you ran slim
pop="sim.AK" # what you named population in slim simulation
# params from /Users/annabelbeichman/Documents/UCLA/Otters/OtterExomeProject/scripts/scripts_20180521/analyses/slim/neutralSimulations/1D.2Epoch.4gen/slim.1D.2Epoch.4gen.array.sh
Nanc_true=4381 # nanc you simulated
nu_true=30 # nu you simulated
T_true=4 # gen you simulated
...
allbest <- NULL
for(rep in seq(1,11)){
  results <- read.table(paste(data.dir,pop,".rep.",rep,".dadi.inf.",dadimodel,".all.output.concatted.txt",sep=""),sep = "\t",header=T)
  # select best LL
...
p4 <- ggplot(allresults,aes(x=rep,y=LL,color="2Epoch"))+
  geom_point(size=2)+

...
colors=list(CA=colorPal[1],BAJ=colorPal[7],AK=colorPal[2],AL=colorPal[3],COM=colorPal[4],KUR=colorPal[5],OR="purple",CA.SM="lightgreen",CA.NIC="lightgreen",CA.AN="darkgreen") # your population colors
ancShape=8
modShape=1
height=5
width=7
###### Doing two sets: all individuals or just low coverage individuals ##########
HCSampleList=read.table("/Users/annabelbeichman/Documents/UCLA/Otters/OtterExomeProject/scripts/scripts_20180521/data_processing/variant_calling_aDNA/bamLists/angsd.bamList.mappedtoElutfullpaths.HighCovPlusADNAOnly.PLUSEXTRAADNA.notIndelRealYet.IDsOnly.txt")
LCSampleList=read.table("/Users/annabelbeichman/Documents/UCLA/Otters/OtterExomeProject/scripts/scripts_20180521/data_processing/variant_calling_aDNA/bamLists/angsd.bamList.LowCoverageOnly.mappedtoElutfullpaths.PLUSEXTRAADNA.notIndelRealYet.IDsOnly.txt")
################################# high coverage ###############################
angsddates=c("20191202-highcov-AFprior-MajorMinor4-IncludesMoreInds-notAllIndelRealignedYet") # low cov for now
refs=c("elut") # set what reference you mapped to 
states=c("1e-06.snpsOnly.TransvOnly")
...
for(angsddate in angsddates){
  ...
  for(ref in refs){
    ...
    for(state in states){
      ...
      # get more fine scale
      bamList$pop2 <- bamList$pop
      bamList[grep("_AN_",bamList$sampleID),]$pop2 <- "CA.AN"
      bamList[grep("_SM_",bamList$sampleID),]$pop2 <- "CA.SM"
      bamList[grep("_NIC_",bamList$sampleID),]$pop2 <- "CA.NIC"
      bamList$captured <- "yes"
      bamList[bamList$sampleID %in% c("A27_Elut_CA_SM_41_SN1_screen","A23_Elut_CA_SM_33_SN2_screen","A21_Elut_CA_SM_32_SN2_screen"),]$captured <- "no"
      #######################################################################
      ...
      PC$pop2 <- factor(unlist(bamList$pop2))
      PC$captured <- factor(unlist(bamList$captured))
      ...
      p12a <- ggplot(PC, aes_string(x=x_axis, y=y_axis)) + 
        geom_point(aes(color=pop2,shape=captured),size=8) +
        ggtitle(title)+
        theme_bw()+
        xlab(xlab)+
        ylab(ylab)+
...
################################# low coverage ###############################
angsddates=c("20191202-lowcov-AFprior-MajorMinor4-IncludesMoreInds-notAllIndelRealignedYet") # high cov only
refs=c("elut") # set what reference you mapped to 
states=c("1e-06.snpsOnly.TransvOnly")
minMafs=c("0.12","0.2","0.05")
# see below for downsampled inds only 

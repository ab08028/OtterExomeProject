...
OR1SampleList=read.table("/Users/annabelbeichman/Documents/UCLA/Otters/OtterExomeProject/scripts/scripts_20180521/data_processing/variant_calling_aDNA/bamLists/OR1.IDList.txt")
OR2SampleList=read.table("/Users/annabelbeichman/Documents/UCLA/Otters/OtterExomeProject/scripts/scripts_20180521/data_processing/variant_calling_aDNA/bamLists/OR2.IDList.txt")
################################# OR 1 ###############################
angsddates=c("20191205-highcov-AFprior-MajorMinor4-justOR1") # low cov for now
...
for(angsddate in angsddates){
  ...
  for(ref in refs){
    ...
    for(state in states){
      ...
      bamList=OR1SampleList # HIGH COVERAGE
      ...
      p12a <- ggplot(PC, aes_string(x=x_axis, y=y_axis)) + 
        geom_point(aes(color=pop2),size=8) +
        ggtitle(title)+
        theme_bw()+
        xlab(xlab)+
        ylab(ylab)+
...
################################# OR 2 ###############################
angsddates=c("20191205-highcov-AFprior-MajorMinor4-justOR2") # low cov for now
refs=c("elut") # set what reference you mapped to 
states=c("1e-06.snpsOnly.TransvOnly")
minMafs=c("0.12","0.2","0.05")
# see below for downsampled inds only 
  ...
  for(ref in refs){
    ...
    for(state in states){
      for(minMaf in minMafs){
        ...
        # from list of bam names that was used in angsd
        # the order of these individuals will match the rows of the PCs 
        bamList=OR2SampleList # HIGH COVERAGE
        colnames(bamList) <- "sampleID"
        #bamList$bamName <- lapply(strsplit(bamList$bamPath,"/"),tail,n=1) # tail pulls out last entry. useful because the different paths have different numbers of dirs so it's not always the same index # but it is the last entry always
        #bamList$sampleID <- lapply(strsplit(as.character(bamList$bamName),"\\."),"[",1)
        bamList$category <- "modern"

...
ref="Elut" # set what reference you mapped to 
...
covar <- npyLoad("/Users/annabelbeichman/Documents/UCLA/Otters/OtterExomeProject/scripts/sandbox/plottingPCA_fromCovMat/pcangsd.cov.npy") # ready from binary ## NOTE; this test dataset is only based on a tiny part of a chromosome. Only 4 snps! I think this test was mapped to elut. let's see
...
bamList <- read.table("/Users/annabelbeichman/Documents/UCLA/Otters/OtterExomeProject/scripts/scripts_20180521/data_processing/variant_calling_aDNA/angsd.bamList.mappedtoElutfullpaths.txt",stringsAsFactors = F)
...
p1 <- ggplot(PC, aes_string(x=x_axis, y=y_axis)) + 
  geom_point(aes(color=category,shape=population),size=8,alpha=0.7) +
  ggtitle(title)+
  theme_bw()+
  geom_text(aes(label=sampleID),stat="identity",size=2)
p1

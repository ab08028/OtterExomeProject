      ...
      for(category in categories){
        for(probCutoff in probCutoffs){
          for(minInd in minInds){
            input <- read.table(paste(data.dir,"angsdOut.mappedTo",ref,".superfile.",category,".hetHomTotals.ProbCutoff.",probCutoff,".DepthCutoff.",depthCutoff,".minInd.",minInd,".",angsdDatePrior,".txt",sep=""),header=T,sep="\t")
...
input_melt <- melt(allInputs,measure.vars = c("HetPerSite","HetPerSite_TransversionsOnly","callableSites","sumHomAltGLsOrGPs","sumHomAltGLsOrGPs_TransversionsOnly"),id.vars = c("sample","label","reference","date","prior","Filter_PerIndividualDepthMinimum","Filter_ProbThresholdForCallableSite","Filter_minIndsPerSite","downsampled"))
...
for(ref in refs){
  p1 <- ggplot(hetOnly_input_melt[hetOnly_input_melt$prior=="AFprior" & hetOnly_input_melt$reference==ref,],aes(x=label,y=value,fill=label))+
    ...
    xlab("")+
    facet_grid(interaction(downsampled,Filter_minIndsPerSite,Filter_PerIndividualDepthMinimum,Filter_ProbThresholdForCallableSite)~TVLabel,scales="free")
  p1
  ...
  # just use filter of 0.95 to simplify results:
    ...
    geom_hline(data=data.frame(yint=0.0003,TVLabel="Transitions+Transversions",downsampled=c("Downsampled","Non-Downsampled"),Filter_PerIndividualDepthMinimum=1),aes(yintercept=yint),linetype="dashed")+ # just adding to Transit+Transv
facet_grid(interaction(downsampled,Filter_PerIndividualDepthMinimum)~TVLabel,scales="free")# just adding to Transit+Transv
  p3
  ...
  p4a <- ggplot(hetOnly_input_melt[hetOnly_input_melt$prior=="AFprior" & hetOnly_input_melt$reference==ref & hetOnly_input_melt$Filter_ProbThresholdForCallableSite==prob & hetOnly_input_melt$Filter_minIndsPerSite==minInd,],aes(x=label,y=value,fill=label))+
    ...
    geom_hline(data=data.frame(yint=0.0003,TVLabel="Transitions+Transversions",downsampled=c("Downsampled","Non-Downsampled"),Filter_PerIndividualDepthMinimum=1),aes(yintercept=yint),linetype="dashed")+ # just adding to Transit+Transv
    facet_grid(TVLabel~downsampled)# just adding to Transit+Transv
  p4a

...
20191202: As a difference, this script doesn't sum up GPs, it actually only takes hard calls where the call is GP > threshold anddivides by callable GTs that have a genotype >thereshold. This should drop out hets I am thinking?
...
hetCountDict=dict()
homAltCountDict=dict()
homRefCountDict=dict()
####  transversions:
TransvOnly_HetCountDict=dict()
TransvOnly_HomAltCountDict=dict()
#TransvOnly_HomRefProbSumDict=dict()
# populate all the dicts with sample IDs and 0s:
for sample in sampList:
    calledDict[sample]=0
    missingDict[sample]=0
    hetCountDict[sample]=0
    homAltCountDict[sample]=0
    homRefCountDict[sample]=0
    TransvOnly_HetCountDict[sample]=0
    TransvOnly_HomAltCountDict[sample]=0
...
# make a dictionary:
        ...
        for sample in sampList:
            ...
            if float(counts_perInd_Dict[sample]) < float(PerIndividualDepthMinimum) or float(max(GTs)) < float(MaxProbCutoff):
                ...
                # get position of the maximum GP:
                maxGPPosition=GTs.index(max(GTs))
                ...
                ##### Need to figure out what the site is called as based on which one is > GP threshold:
                if maxGPPosition==0:
                    homRefCountDict[sample]+=float(1) # add 1 to the total count
                elif maxGPPosition==1:
                    hetCountDict[sample]+=float(1) # add 1 to the total count
                elif maxGPPosition==2:
                    homAltCountDict[sample]+=float(1) # add it to the total probability 
    # check if it's a transversion, if yes, add to the transvHetDict, if not don't
                if (allele1,allele2) in transversions:
                    ...
                    if maxGPPosition==1:
                        TransvOnly_HetCountDict[sample]+=float(1) # add 1 to the total count
                    elif maxGPPosition==2:
                        TransvOnly_HomAltCountDict[sample]+=float(1)
                    else:
                        continue
...
for sample in sampList:
    heterozygosityDict[sample] = float(hetCountDict[sample])/calledDict[sample]

TransvOnly_heterozygosityDict=dict()
for sample in sampList:
    TransvOnly_heterozygosityDict[sample] = float(TransvOnly_HetCountDict[sample])/calledDict[sample]
...
for sample in sampList:
    out=[sample,str(missingDict[sample]),str(calledDict[sample]),str(hetCountDict[sample]),str(TransvOnly_HetCountDict[sample]),str(homAltCountDict[sample]),str(TransvOnly_HomAltCountDict[sample]),str(homRefCountDict[sample]),str(heterozygosityDict[sample]),str(TransvOnly_heterozygosityDict[sample]),str(PerIndividualDepthMinimum),str(minIndsPerSite),str(MaxProbCutoff)]

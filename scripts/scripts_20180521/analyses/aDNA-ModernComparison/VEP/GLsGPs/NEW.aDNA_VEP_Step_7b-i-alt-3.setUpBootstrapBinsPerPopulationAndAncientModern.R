...
#for(bin in seq(1,10)){
  ...
  for(ind in seq(0,length(bamList$sample)-1)){
    ...
    if(as.numeric(totalCallableSites)>=as.numeric(minCalledSitesPerWindowPerIndividual)){
  ...
  if(as.numeric(totalIndsWithData)>=as.numeric(minIndPerWindow)){
    # add that info to df
    ...
    ## need to then average over those: ### this is the step I don't want to do any more (do at end of bootstrapping)
   # summaries <- binTotals %>% 
   ...
   #   summarise(avgRescaledHomRef=mean(sumHomRef_Rescaled,na.rm=T),avgRescaledHet=mean(sumHet_Rescaled,na.rm=T),avgRescaledHomAlt=mean(sumHomAlt_Rescaled,na.rm=T))
    ...
    ### add metadata: to binTotals instead of summaries:
    binTotals$binNum <- bin
    binTotals$scaff <- as.character(unlist(seqnames(bins[bins$binNum==bin,])))
    binTotals$start <- start(bins[bins$binNum==bin,])
    binTotals$end <- end(bins[bins$binNum==bin,])
    binTotals$width <- width(bins[bins$binNum==bin,])
    binTotals$minDepth <- minDepth
    binTotals$minGP <- minGP
    binTotals$minCalledSitesPerWindowPerIndividual <- minCalledSitesPerWindowPerIndividual
    binTotals$minIndPerWindow <- minIndPerWindow
    binTotals$totalIndsWithData <- totalIndsWithData
    # finally: add to allBinsallInds df:
    allBinsallInds <- rbind(allBinsallInds,data.frame(binTotals))
...
# write out bin sums: 
write.table(allBinsallInds,paste(out.dir,"/",outPREFIX,".Modern.Ancient.SumsPerIndividuals.RescaledPerWindow.binSize.",binsize,".minIndPerWindow.",minIndPerWindow,".minCallSitesPerWindow.",minCalledSitesPerWindowPerIndividual,".txt",sep=""),col.names = T,row.names = F,quote=F,sep="\t")

...
for(angsdDate in dates){
  data.dir=paste("/Users/annabelbeichman/Documents/UCLA/Otters/OtterExomeProject/results/analysisResults/aDNA-ModernComparison/VEP/GLsGPs/compareMisSynDists_withBootstraps/",angsdDate,"/PointEstsPlusBootstraps/",sep="")
  ...
  variance_mod <- variances[variances$group=="Modern",]
  pointEst_anc <- pointEst[pointEst$group=="Ancient",c("sites","Consequence_BroadName","homAlt","derivedAlleles")]
  pointEst_mod <- pointEst[pointEst$group=="Modern",c("sites","Consequence_BroadName","homAlt","derivedAlleles")]
  # put together point ests and variance for modern/ancient:
  modCombo <- merge(pointEst_mod,variance_mod,by=c("sites","Consequence_BroadName"))

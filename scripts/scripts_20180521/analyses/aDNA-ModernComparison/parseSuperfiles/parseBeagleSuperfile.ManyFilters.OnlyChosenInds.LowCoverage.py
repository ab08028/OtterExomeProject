...
# 20191014 -- trying a modification where I only consider one ancient individual (A30) and one CA individual (116) -- can be adjusted to consider any sets of individuals

usage: python script.py inputFilepath sampleIDFile outputFile MaxProbCutoff PerIndividualDepthMinimum 
"""
import gzip
import sys
...
#outname="/Users/annabelbeichman/Documents/UCLA/Otters/OtterExomeProject/scripts/sandbox/parseBeagleSuperfile/testout.ChosenInds.txt"
#MaxProbCutoff=0.95 # if the max of the 3 probs is below this, discard; keep if >= to the cutoff
#PerIndividualDepthMinimum=1
#minIndsPerSite=2
chosenADNAInds=["A30_Elut_CA_SM_35_SN1_CAP"]
chosenModernInds=["116_Elut_CA_307_downsamp"] # downsampled
chosenInds_combo=chosenADNAInds+chosenModernInds
### work on filters #####
...
chosenADNA_Indices = [sampList.index(x) for x in chosenADNAInds]
chosenMODERN_Indices = [sampList.index(x) for x in chosenModernInds]
chosenInds_comboIndices = chosenADNA_Indices + chosenMODERN_Indices # both ancient and modern
...
for line0 in superfile:
    ...
    chosen_counts=[counts[x] for x in chosenInds_comboIndices]
            # iterate through the individuals
        # get max per set:
        # want to get count information:
        # make a dictionary of the read counts per individual:
    chosen_counts_perInd_Dict = dict(zip(chosenInds_combo,chosen_counts))
    #### get chosen individuals:
    ...
    ### ----> chosenIndsPassingThreshold = sum(float(i) >= float(PerIndividualDepthMinimum) for i in chosenCounts)
    # need to check if a minimum number of counts pass the count threshold
    ############# 20191014: trying a filter where I require the inds in my chosen lists to have coveragine passing the threshold 
    ## check if 
   ...
   # if float(chosenIndsPassingThreshold) < len(chosenInds_comboIndices):
   #     continue # skip this site if the chosen individuals aren't called
    #elif float(chosenIndsPassingThreshold) >= len(chosenInds_comboIndices):
...
# make a dictionary:
    chosen_GTs_perInd= [GTs_perInd[x] for x in chosenInds_comboIndices]

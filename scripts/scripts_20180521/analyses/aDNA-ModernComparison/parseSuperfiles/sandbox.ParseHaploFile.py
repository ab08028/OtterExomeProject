...
Want to filter:
1) non bi-allelic sites (note that from this kind of file you dont' know what the reference allele is. so it's possible that if CC and GG are observed that the reference is T. Not worrying about this small category of sites for now, because at least it's biallelic within the otter samples, but be aware -- Fages doesn't deal with these at all, so I'm not *too* worried about it)
...
Result: transversions + monomorphic sites only 
...
for line0 in haploFile:
    ...
    # a few checks
    # first if the length of the set after Ns removed is 1, then it's monomorphic and you can write it out and move on

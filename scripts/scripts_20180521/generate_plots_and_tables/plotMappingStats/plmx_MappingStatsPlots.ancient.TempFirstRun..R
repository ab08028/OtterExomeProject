...
date=20180720 # date of stats
stats <- read.table(paste(resultsDir,"QC_Reports/plmx_mappingStats/",date,"/ancient_FirstRun_noIndelR_20180712/plmx.aDNA.summary.stats.firstRun.txt",sep=""),header=T,stringsAsFactors = F)
...
p0
ggsave(paste(plotoutDir,"aDNA.RetainedReads.",date,".pdf",sep=""),p0,device="pdf",width = 8,height=8)
...
p1b <- ggplot(data=statsToPlot[statsToPlot$simpleRef=="ferret",],aes(x=reorder(sample,value),y=value))+
  geom_bar(stat="identity")+
  facet_wrap(~statistic,scales = "free_x")+
  coord_flip()+
  xlab("")+
...
stat.comp <- stats[stats$simpleRef=="ferret" | stats$reference=="noREF",c("sample","statistic","value")]

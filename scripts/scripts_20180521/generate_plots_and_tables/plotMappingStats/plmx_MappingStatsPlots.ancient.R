...
statsToPlot1 <- stats[stats$statistic!="hits_raw" & stats$statistic!="hits_raw_frac"  & !(stats$statistic %in% c("MD_001","MD_002","MD_003")) & !(stats$statistic %in% c("Mb >= 3x")),]
statsToPlot1$statistic <- factor(statsToPlot1$statistic)
levels(statsToPlot1$statistic) <- c("PCR Duplicate (frac)","Mapped Read Length\n(bp)", "Unique Mapped Reads\n(read count)","Unique Mapped Reads\n(frac)","Mb >= 1x","Mb >= 2x","Sequencing Reads")
...
p0b <- ggplot(data=statsToPlot1[statsToPlot1$statistic %in% c("Mb >= 1x","Mb >= 2x"),],aes(x=reorder(sample,plotOrder),y=value,fill=statistic))+
...
# how do I save these?
ggsave(paste(plotoutDir,"aDNAMappingStats",todaysdate,".pdf",sep=""),p1_grid,device="pdf",width = 8,height=16)
...
p1b <- ggplot(data=statsToPlot[statsToPlot$simpleRef=="sea otter",],aes(x=reorder(sample,value),y=value))+
  ...
  theme_bw()
p1b
ggsave(paste(plotoutDir,"aDNA.AllSamples.Stats.MfurRef.",todaysdate,".pdf",sep=""),p1b,device="pdf",width = 8,height=8)
